...
- Added a low-friction Codex skill that recommends repository licensing from
  practical permission goals and current rights evidence.
- Preserved the user's final choice while limiting clarification and
  alternatives to decisions that can materially change the result.
- Covered open source, source-available and no-license outcomes; layered path
  maps; upstream, derivative and contributor rights; and forward-only changes.
- Added Codex UI metadata and standalone installation instructions.
- Applied layered SUL-1.0 and CC BY-NC-SA 4.0 licensing with a path-level map
  and no-assignment contribution policy.
